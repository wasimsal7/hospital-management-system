...
const getAvailability = async () => {
  try {
    ...
    const response = await axios.get('http://localhost:5000/user/doctor/availability', {
      headers: { 
...
const setAvailability = async () => {
  try {
    const token = localStorage.getItem('token');
    const response = await axios.post('http://localhost:5000/user/doctor/availability', available.value, {
      headers: { 
        'Authorization': `Bearer ${token}` 
      },
    });
